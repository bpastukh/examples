Feature('@first Create Todos @step:06 @smoke @story:12345 @S50705c91')
 ...
 * Happy Path tests
 */
Scenario('Create a new todo item @Tf273e309', async ({ I, TodosPage }) => {
...
Scenario('Create multiple todo items @T10252834', async ({ I, TodosPage }) => {
  I.say('Given I have an empty todo list')
...
Data(examples).
Scenario('Todos containing weird characters @Td8a15bff', async ({ I, current, TodosPage }) => {
...
Scenario('Text input field should be cleared after each item @T350164bd', async ({ I, TodosPage }) => {
  I.say('Given I have an empty todo list')
...
Scenario('Text input should be trimmed @T39d188be', async ({ I, TodosPage }) => {
  I.say('Given I have an empty todo list')
...
Scenario('New todos should be added to the bottom of the list @T4bca583f', async ({ I, TodosPage }) => {
...
Scenario('Footer should be visible when adding TODOs @T4c500955', async ({ I, TodosPage }) => {
